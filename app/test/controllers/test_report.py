...
def test_get_all(app, create_repeted_clients_and_ingredients_order):
    ...
    data = report_from_db['client_data']['clients'][0]['client_name']
    pytest.assume(error is None)
    pytest.assume(report_from_db['ingredient']['id'] == most_repeated_ingredient_id)
    pytest.assume(report_from_db['client_data']['clients'][0]['client_name'] == most_repeated_client["client_name"])
    pytest.assume(report_from_db['client_data']['clients'][1]['client_name'] == second_most_repeated_client["client_name"])
    pytest.assume(report_from_db['client_data']['clients'][2]['client_name'] == third_most_repeated_client["client_name"])
